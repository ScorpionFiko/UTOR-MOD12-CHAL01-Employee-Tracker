...
const SUB_MENU_34 = 34;
const SUB_MENU_35 = 35;
...
// update role flags
let updateRoleTitle = false;
let updateRoleSalary = false;
let updateRoleDepartment = false;
...
class CLI {
  constructor() {
    ...
    this.roleMenuPrompts = [
      {
        ...
        choices: [
          ...
          { name: "Update role title", value: SUB_MENU_33 },
          { name: "Update role salary", value: SUB_MENU_34 },
          { name: "Update role department", value: SUB_MENU_35 },
    ...
    this.updateRoleInfoPrompts = [
      ...
      },
      {
        ...
        when: () => { return updateRoleSalary }
      },
      {
        type: 'list',
        message: 'Select new department to which this role belongs',
        name: 'updateRoleDepartment',
        choices: () => this.getDepartmentChoices(),
        when: () => { return updateRoleDepartment }
      }
    ];
  ...
  //           break;
  //         case SUB_MENU_14:
  //           this.updateDepartment(menuOptions);
  //           break;
  //         //role menus
  //         case SUB_MENU_30:
  ...
  //           break;
  //         case SUB_MENU_33:
  //           updateRoleTitle = true;
  //           this.updateRoleInfo(menuOptions);
  //           break;
  //         case SUB_MENU_34:
  //           updateRoleSalary = true;
  //           this.updateRoleInfo(menuOptions);
  //           break;
  //         case SUB_MENU_35:
  //           updateRoleDepartment = true;
  //           this.updateRoleInfo(menuOptions);
  //           break;
  //         //employee menus
  //         case SUB_MENU_60:
  ...
  // }
  // updateDepartment(menuOptions) {
  //   return inquirer.prompt(this.updateDeptInfoPrompts).then((answers) => {

  //     const sql = "update department set name=? where id=?";
  //     const params = [];
  //     params.push(answers.updateDept);
  //     params.push(answers.updateDeptList.id)
  //     return this.db.promise().query(sql, params)
  //       .then(([rows, fields]) => {
  //         console.log(`Updated \x1b[42m${answers.updateDeptList.name}\x1b[0m information in the Database`);
  //       }).catch((err) => {
  //         console.log(`>>> Error has occured in running the query: ${sql}`);
  //       })
  //       .then(() => this.presentSubMenu(menuOptions))
  ...
  // update roles
  updateRoleInfo(menuOptions) {
    return inquirer.prompt(this.updateRoleInfoPrompts).then((answers) => {
      let sql = "";
      const params = [];
      if (updateRoleTitle) {
        sql = "update role set title=? where id= (?)";
        params.push(answers.updateRoleTitle);
      }
      if (updateRoleSalary) {
        sql = "update role set salary=? where id= (?)";
        params.push(answers.updateRoleSalary);
      }
      if (updateRoleDepartment) {
        sql = "update role set department_id=? where id= (?)";
        params.push(answers.updateRoleDepartment.id);
      }
      params.push(answers.updateRole.id);
      this.db.promise().query(sql, params)
        .then(([rows, fields]) => {
          console.log(`Updated \x1b[42m${answers.updateRole.name}\x1b[0m information in the Database`);
        }).catch((err) => {
          console.log(`>>> Error has occured in running the query: ${sql}`);
        })
        .then(() => {
          this.presentSubMenu(menuOptions)
          updateRoleTitle = false;
          updateRoleSalary = false;
          updateRoleDepartment = false;
        })
    })
  }
