...
class Employee {
  constructor(cli) {
    ...
    this.emplMenuPrompts = [
      {
        type: "list",
        message: `\x1b[46mEmployee Menu\x1b[0m\nWhat would you like to do?`,
        name: "subMenuCommand",
        choices: [
    ...
    this.emplInfoPrompts = [
      {
        type: "list",
        message: () => {
          return `Select the employee you would like to ${this.delete ? `delete` : `update`
            }`;
        },
        name: "emplList",
      ...
      },
      {
        type: "input",
        message: () => {
          return `Enter ${this.add ? `` : `a new `}employee\'s first name:`;
        },
        name: "emplFirstName",
        when: () => {
          return this.add || this.updateName;
        },
      },
      {
        type: "input",
        message: () => {
          return `Enter ${this.add ? `` : `a new `}employee\'s last name:`;
        },
        name: "emplLastName",
        when: () => {
          return this.add || this.updateName;
      ...
      },
      {
        type: "list",
        message: () => {
          return `Select employee\'s ${this.add ? `` : `a new `}role`;
        },
        name: "emplRole",
        choices: async () => await new Role().getRoleChoices(),
        when: () => {
          return this.add || this.updateRole;
        },
      },
      {
        type: "list",
        message: () => {
          return `Select employee\'s ${this.add ? `` : `a new `}manager`;
        },
        name: "emplManager",
        choices: async () => await this.getEmployeeChoices(true),
        when: () => {
          return this.add || this.updateManager;
        },
      },
    ];
  }
  ...
  presentSubMenu() {
    return inquirer.prompt(this.emplMenuPrompts).then((answers) => {
  ...
  addEmployee() {
    return inquirer.prompt(this.emplInfoPrompts).then((answers) => {
      ...
      this.db
        ...
        .then(([rows, fields]) => {
          console.log(
            `Added \x1b[42m${answers.emplFirstName} ${answers.emplLastName
            }\x1b[0m as an employee in the role of \x1b[42m${answers.emplRole.name
            }\x1b[0m with \x1b[42m${answers.emplManager.name}\x1b[0m ${answers.emplManager.id === null ? `` : `as manager `
            }to the Database\n`
          );
        })
        .catch((err) => {
  ...
  deleteEmployee() {
    return inquirer.prompt(this.emplInfoPrompts).then((answers) => {
      ...
      this.db
        ...
        .then(([rows, fields]) => {
          console.log(
            `Deleted employee \x1b[42m${answers.emplList.name}\x1b[0m from the Database\n`
          );
        })
        .catch((err) => {
          console.log(`>>> Error has occured in running the query: ${sql}\n`);
  ...
  updateEmployeeInfo() {
    return inquirer.prompt(this.emplInfoPrompts).then((answers) => {
      ...
      this.db
        ...
        .then(([rows, fields]) => {
          console.log(
            `Updated \x1b[42m${answers.emplList.name}\x1b[0m\'s information in the Database\n`
          );
        })
        .catch((err) => {
          console.log(`>>> Error has occured in running the query: ${sql}\n`);
